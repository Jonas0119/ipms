...
- **新增模块**：`kjhlog` - 自研日志工具，替代 `ipms-log` 依赖
...
### 容器 IP 地址分配
- **MySQL (mysql57)**: `172.18.0.2:3306`
- **Redis (my-redis)**: `172.18.0.3:6379`
- **MinIO (minio)**: `172.18.0.4:9000/9001`
- **API Server (api-server1)**: `172.18.0.5:6688`
- **Nginx (my-nginx1)**: `172.18.0.6:80`

### 服务访问地址
- **前端控制台**: http://localhost:8080
- **API 服务**: http://localhost:6688
...
# 查看特定服务日志
docker-compose logs -f api-server1
...
# 重启特定服务
docker-compose restart api-server1
```

### 进入容器
```bash
# 进入 API 服务器容器
docker exec -it api-server1 /bin/bash
...
```yaml
ports:
  - "8080:80"  # 改为其他端口，如 "8081:80"
```

### 2. 服务启动失败
```bash
# 查看详细日志
docker-compose logs api-server1
