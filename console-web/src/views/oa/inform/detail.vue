        ...
        <Card dis-hover :bordered="false" class="mt-16">
            <div class="article">
                ...
                <h6>
                    <span>
                        撰稿人：
                        <router-link :to="`/hr/colleague/detail/${detail.created_by}`">
                            {{ detail.real_name }}
                        </router-link>
                    </span>
                    <span v-if="detail.published">
                        发布人：
                        <router-link :to="`/hr/colleague/detail/${detail.published_by}`">
                            {{ detail.published_real_name }}
                        </router-link>
                    </span>
...
export default {
    ...
    methods: {
        ...
        doPublish() {
            Modal.confirm({
                title: '请确认',
                content: `确认要发布「${this.detail.title}」行政通知吗？发布后将不可修改`,
                onOk: () => {
                    const data = {
    ...
    computed: {
        ...
        num() {
            return utils.order.num('IA', this.detail.created_at, this.detail.id);
        }
    }
};
</script>
