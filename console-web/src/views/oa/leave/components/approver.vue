...
export default {
            ...
            columns: [
                {
                    title: '流程编号',
                    minWidth: 140,
                    render: (h, p) => h('span', utils.order.num('LF', p.row.created_at, p.row.id))
                },
                {
                    title: '申请人',
                    minWidth: 120,
                ...
                },
                {
                    title: '请假时间',
                    minWidth: 220,
                    render: (h, p) => h('span', `${moment(p.row.begin_date).format('YYYY-MM-DD')}`)
                },
                {
                    title: '请假天数',
                    minWidth: 8,
                ...
                },
                {
                    ...
                    render: (h, p) =>
                        h(
                            'a',
                            { on: { click: () => this.goDetail(p.row.id) } },
                            p.row.success === null && !p.row.cancel ? '立即审批' : '查看'
                        )
                }
            ]
        };
    ...
    methods: {
        ...
        goDetail(id) {
            this.$router.push(`/oa/leave/detail/${id}`);
        }
    },
    computed: {
        ...mapGetters({
