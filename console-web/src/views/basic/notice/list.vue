...
export default {
            ...
            columns: [
                {
                    title: '编号',
                    minWidth: 120,
                    render: (h, p) => h('span', utils.order.num('N', p.row.created_at, p.row.id))
                },
                {
                    title: '标题',
                    minWidth: 120,
                ...
                },
                {
                    ...
                    render: (h, p) =>
                        h(
                            Tag,
                            { props: { color: p.row.notice_tpl_id ? 'success' : 'default' } },
                            p.row.notice_tpl_id ? '是' : '否'
                        )
                },
                {
                    title: '发布状态',
    ...
    methods: {
        ...
        goDetail(id) {
            this.$router.push(`/basic/notice/preview/${id}`);
        },
        goUpdate(id) {
            this.$router.push(`/basic/notice/update/${id}`);
        },
        doPublish(record, index) {
            Modal.confirm({
                title: '请确认',
                content: `确认要发布「${record.title}」通知吗？发布后将不可修改`,
                onOk: () => {
                    const data = {
