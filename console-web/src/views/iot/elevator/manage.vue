...
<template>
    <WaterMark>
        ...
        <Button type="primary" @click="showDrawer">添加梯控</Button>
        ...
        >
            <Form :model="form" :rules="rules" ref="form" @submit.native.prevent>
                <FormItem label="梯控名称：" prop="name">
                    <Input placeholder="请输入梯控名称" v-model="form.name" />
                </FormItem>
                ...
                <FormItem label="识别码：" prop="sign">
                    <Input placeholder="请输入梯控识别码" v-model="form.sign" />
                </FormItem>
                ...
                <FormItem label="物业费未缴清禁止乘梯：" prop="verify_property_fee">
    ...
    data() {
        return {
            ...
            rules: {
                ...
                name: [
                    { required: true, message: '请输入梯控名称' },
                    { max: 56, message: '梯控名称不能超过56个字符' }
                ],
                building: [
                    { required: true, message: '请输入所属楼栋' },
                    { max: 32, message: '所属楼栋不能超过32个字符' }
                ],
                category: [{ required: true, type: 'number', message: '设备分类' }],
                verify_property_fee: [{ required: true, type: 'number', message: '请选择是否物业费未缴清禁止乘梯' }],
